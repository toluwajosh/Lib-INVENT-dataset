"""
This file has been modified by Joshua Owoyemi on 15th March 2023.
    Original file was licensed under Apache License, Version 2.0
...
from reinvent_chemistry.standardization.rdkit_standardizer import (
    RDKitStandardizer,
)
from reinvent_chemistry.standardization.filter_configuration import (
    FilterConfiguration,
)
...
class StatsExtractor:
    ...
    def __init__(self, configuration: StatsExtractionConfig):
        ...
        standardisation_config_dict = (
            self._configuration.standardisation_config
        )
        ...
        self._mol_wts_udf = psf.udf(
            lambda x: ExactMolWt(Chem.MolFromSmiles(x)) if x else 1e100,
            pst.FloatType(),
        )
        ...
        self._num_atoms_udf = psf.udf(
            lambda x: Chem.MolFromSmiles(x).GetNumHeavyAtoms() if x else 1e100,
            pst.IntegerType(),
        )
        ...
        self._hetero_atom_ratio_udf = psf.udf(
            lambda x: len(
                [
                    atom
                    for atom in Chem.MolFromSmiles(x).GetAtoms()
                    if atom.GetAtomicNum() == 6
                ]
            )
            / Chem.MolFromSmiles(x).GetNumHeavyAtoms()
            if x
            else 1e100,
            pst.FloatType(),
        )
        self._make_canonical_udf = psf.udf(
            lambda x: Chem.MolToSmiles(Chem.MolFromSmiles(x)), pst.StringType()
        )
        ...
        self._tokeniser_udf = psf.udf(
            self.regex.findall, pst.ArrayType(pst.StringType())
        )
        self._decoration_split_udf = psf.udf(
            lambda x: x.split(dec_separator), pst.ArrayType(pst.StringType())
        )
    ...
    def _number_attachment_points(self, df: ps.DataFrame) -> pd.DataFrame:
        attachments_histogram = (
            df.withColumn(
                self._stats.NUMBER_OF_DECORATIONS,
                self._count_decorations_udf(self._columns.SCAFFOLDS),
            )
            .groupBy(self._stats.NUMBER_OF_DECORATIONS)
            .count()
            .toPandas()
            .set_index(self._stats.NUMBER_OF_DECORATIONS)
            .sort_index()
        )
    ...
    def _count_decorations(self, df: ps.DataFrame) -> None:
        ...
        save_path = os.path.join(
            save_folder, f"{self._stats.NUMBER_OF_DECORATIONS}.csv"
        )
        df.to_csv(save_path)

        if self._configuration.plotting:
            self._plot_and_save(
                df, self._stats.NUMBER_OF_DECORATIONS, bar=True
            )
    ...
    def _compute_single_property(
        self, df: ps.DataFrame, property: str, column
    ) -> ps.DataFrame:
    ...
    def _compute_all_stats(
        self, df: ps.DataFrame, methods: List[str], column
    ) -> ps.DataFrame:
        for method in methods:
            df = self._compute_single_property(df, method, column)
        return df

    def update_token_dictionary(
        self, token_dictionary: Dict, character_list: List[str]
    ) -> Dict:
    ...
    def token_distribution(
        self, df: ps.DataFrame, column: str
    ) -> pd.DataFrame:
        token_dict = dict()
    ...
    def _plot_and_save(
        self, df: pd.DataFrame, name: str, bar: bool = False
    ) -> None:
        if bar:
            df.plot.bar()
        else:
        ...
        save_path_image = os.path.join(
            self._configuration.output_path, f"plots/{name}.png"
        )
    ...
    def _make_folders(self):
        ...
        if self._configuration.plotting:
            save_folder = os.path.join(
                self._configuration.output_path, "plots"
            )
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)

    def _compute_stats_for_property(
        self, df: ps.DataFrame, property: str, column: str
    ) -> None:
        print(f"Working on {property}")
        pandas_df = (
            df.groupBy(property)
            .agg(psf.count(property).alias("counts"))
            .toPandas()
            .set_index(property)
            .sort_index()
        )
        save_path = os.path.join(
            self._configuration.output_path, f"data/{property}_{column}.csv"
        )
        pandas_df.to_csv(save_path)
        if self._configuration.plotting:
            self._plot_and_save(pandas_df, f"{property}_{column}", bar=False)

    def _compute_token_distribution_stats(
        self, df: ps.DataFrame, column: str
    ) -> None:
        print("Working on token distribution")
        token_df = self.token_distribution(df, column)
        save_path = os.path.join(
            self._configuration.output_path,
            f"data/{self._stats.TOKEN_DISTRIBUTION}_{column}.csv",
        )
        token_df.to_csv(save_path)
        if self._configuration.plotting:
            self._plot_and_save(
                token_df,
                name=f"{self._stats.TOKEN_DISTRIBUTION}_{column}",
                bar=True,
            )

    def _compute_token_atom_ratio(self, df: ps.DataFrame, column: str) -> None:
        token_numbers = df.select(column)
        token_numbers = token_numbers.withColumn(
            "token_list", self._tokeniser_udf(column)
        )
        token_numbers = token_numbers.withColumn(
            self._stats.NUMBER_OF_TOKENS, psf.size("token_list")
        )

        self._compute_stats_for_property(
            token_numbers, self._stats.NUMBER_OF_TOKENS, column
        )
        if self._stats.NUMBER_OF_ATOMS not in self._configuration.properties:
            ...
            df = df.withColumn(
                self._stats.TOKEN_ATOM_RATIO,
                df[self._stats.NUMBER_OF_TOKENS]
                / df[self._stats.NUMBER_OF_ATOMS],
            )
            self._compute_stats_for_property(
                df, self._stats.TOKEN_ATOM_RATIO, column
            )
    ...
    def _filter_data(self, df: ps.DataFrame) -> ps.DataFrame:
        for prop, condition in self._configuration.filter.items():
            if prop not in df.columns:
                ...
                rule = " ".join(
                    [prop]
                    + [self._purging.MAPPING[condition[0]]]
                    + [str(condition[1])]
                )
                print(rule)
                df = df.where(rule)
    ...
    def prepare_single_column_data(self) -> ps.DataFrame:
        ...
        data = (
            data.select(
                self._standardise_smiles_udf("_c0").alias(
        ...
        if self._configuration.save_standardised:
            data.coalesce(1).write.format("text").option(
                "header", "false"
            ).mode("append").save(self._configuration.output_path)
        return data

    def prepare_sliced_data(
        self, data: ps.DataFrame, column: str
    ) -> ps.DataFrame:
        self._make_folders()
        if column == self._columns.DECORATIONS:
            data = data.select(
                psf.explode(
                    self._decoration_split_udf(self._columns.DECORATIONS)
    ...
    def run_computations(self, data: ps.DataFrame, column: str) -> None:
        data_computed = self._compute_all_stats(
            data, self._configuration.properties, column
        )
        data_computed = self._filter_data(data_computed)

        if self._configuration.save_cut_precomputed:
            print("****about to save****")
            data_computed.select(column).coalesce(1).write.format(
                "text"
            ).option("header", "false").mode("append").save(
                self._configuration.output_path
            )
        ...
        for _method in self._configuration.properties:
            self._compute_stats_for_property(
                data_computed, _method, column=column
            )
    ...
    def run(self):
        ...
        if self._configuration.mode == self._stats.SINGLE_COLUMN_DATA_MODE:
            ...
            data = (
                ...
                .withColumnRenamed("_c0", self._columns.SCAFFOLDS)
                .withColumnRenamed("_c1", self._columns.DECORATIONS)
